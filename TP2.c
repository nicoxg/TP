...
static int parse_comando(const char *linea, char *comando, char *clave, char *valor) {
    int camposOk = sscanf(linea, "%15s %2046s %2046[^\n]", comando, clave, valor);
...
static void atender_cliente(FILE *in, FILE *out) {
    ...
    while (fgets(linea, sizeof(linea), in) != NULL) {
        ...
        if (strcmp(comando, "SET") == 0) {
            cmd_set(clave, valor, out);
        } else if (strcmp(comando, "GET") == 0) {
            cmd_get(clave, valor, sizeof(valor), out);
...
int main(void) {
    ...
    while (1) {
        ...
        // Stream de lectura
        FILE *input = fdopen(fd_cliente, "r");
        if (input == NULL) {
            perror("ERROR: fdopen en la entrada");
            close(fd_cliente);
            continue;
        }

        // Stream de escritura
        // Copia del FD para evitar cerrar el socket  por completo
        int fd_output = dup(fd_cliente);
        if (fd_output == -1) {
            perror("ERROR: dup");
            fclose(input);
            continue;
        ...
        FILE *output = fdopen(fd_output, "w");
        if (output == NULL) {
            perror("ERROR: fdopen en la salida");
            fclose(input);
            close(fd_output);
            continue;
        }

        atender_cliente(input, output);
        fclose(input);
        fclose(output);

        // Uso de la flag signal_sigusr1 para saber si la desconexión fue
        // causada por SIGUSR1 o por el cliente
        if (signal_sigusr1) {
            fprintf(stderr, "INFO: conexion cerrada por SIGUSR1\n");
            signal_sigusr1 = 0;
        }
